...
class LinearICEImputer(BaseMLImputer, ICEImputerMixin):
    ...
    def set_imp_model_params(
        self, updated_model_dict: OrderedDict, params: dict
    ) -> None:
    ...
    def get_imp_model_params(self, params: dict) -> OrderedDict:
        ...
        try:
            parameters = np.concatenate(
                [imp_model.coef_, np.expand_dims(imp_model.intercept_, 0)]
            )
    ...
    def fit(
        ...
        coef = np.concatenate(
            [estimator.coef_, np.expand_dims(estimator.intercept_, 0)]
        )
        ...
        self.fit_res_history[feature_idx].append(
            {"coef": coef, "loss": loss, "sample_size": X_train.shape[0]}
        )

        return {"coef": coef, "loss": loss, "sample_size": X_train.shape[0]}
    ...
    def impute(
        ...
        imputed_values = np.clip(
            imputed_values, min_values[feature_idx], max_values[feature_idx]
        )
